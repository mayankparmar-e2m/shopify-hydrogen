...
import { generateFilterUrlFromSelectedFilters, getFilterLink } from "~/utils/utils";

export default function FilterValues({ onSetSelectedFilters, selectedFilters, filterValues = [], filter, appliedFilters = [] }) {
    const [params] = useSearchParams();
    const location = useLocation();
    const navigate = useNavigate()
    const filterURL = (filterType, singlefilter) => {
        const { selectedFilters: selectedFiltersArray, url } = generateFilterUrlFromSelectedFilters(singlefilter, selectedFilters)
        ...
        <ul className="filter-value_wrapper">
            {
                filterValues.map((filterValue) => {
                    ...
                    const activeFilter = selectedFilters.findIndex((item) => item.label === filterValue.label) >= 0 || appliedFilters.findIndex((item) => item.label === filterValue.label) >= 0
