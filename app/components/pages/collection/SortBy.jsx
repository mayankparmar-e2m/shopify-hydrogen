...
export default function SortBy() {
  ...
  const activeItem =
    SORT_OPTIONS.find((item) => item.key === params.get('sort')) ||
    SORT_OPTIONS[0];
  const selectSortBy = () => {
  ...
  return (
    <div
      className="collection-sortby-wrapper relative md:w-[22%]"
      ref={wrapperRef}
    >
      <div
      ...
      <div
        className={`sort-option-wrapper absolute left-7 mt-3 rounded-lg p-3 border border-solid border-primary bg-white ${
          showSortOption ? 'block' : 'hidden'
        }`}
      >
        <ul>
          {SORT_OPTIONS.map((item) => {
            return (
              ...
              >
                <div
                  onClick={() =>
                    navigate(getSortLink(item.key, params, location))
                  }
                  preventScrollReset
